...
func (_q *MemberQuery) QueryPointAllocations() *PointAllocationQuery {
...
func (_q *MemberQuery) WithPointAllocations(opts ...func(*PointAllocationQuery)) *MemberQuery {
	query := (&PointAllocationClient{config: _q.config}).Query()
...
func (_q *MemberQuery) loadPointAllocations(ctx context.Context, query *PointAllocationQuery, nodes []*Member, init func(*Member), assign func(*Member, *PointAllocation)) error {
	...
	query.Where(predicate.PointAllocation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(member.PointAllocationsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.point_allocation_member
		if fk == nil {
			return fmt.Errorf(`foreign-key "point_allocation_member" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
